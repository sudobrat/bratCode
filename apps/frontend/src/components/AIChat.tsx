...
import type { ChatMessage } from "@bratCode/zod";
...
type ToolType = keyof typeof TOOL_META;
...
const ToolBadge = ({ toolType, detail }: ToolBadgeProps) => {
    if (!toolType || !(toolType in TOOL_META)) return null;
    const meta = TOOL_META[toolType as ToolType];
...
function AiChat({
    ...
    const onEvent = async (toolType: string, data: any) => {
