...
import { llm } from "./llm";
import { Graph } from "@bratCode/zod";
...
const getRecentMessages = (messages: BaseMessage[] = []) => {
    ...
    if (firstUserMessage && !recents.includes(firstUserMessage)) {
        return [firstUserMessage, ...recents];
    }
    return recents;
};

export const graph = ({ projectId, userId }: Graph) => {
    const tools = fileTools({ id: projectId, userId });
    ...
    const agent = async (state: typeof MessagesAnnotation.State) => {
        ...
        const messages = [new SystemMessage(system_prompt), ...recentMessages];
